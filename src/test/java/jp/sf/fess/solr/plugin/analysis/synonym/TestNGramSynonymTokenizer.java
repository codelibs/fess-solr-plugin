...
import jp.sf.fess.solr.plugin.analysis.synonym.NGramSynonymTokenizer.MyToken;
...
public class TestNGramSynonymTokenizer {
    ...
    public void testGetNextBlockLong() throws Exception {
        String src1 = getLengthDummyBlock(
                NGramSynonymTokenizer.BUFFER_SIZE - 2, 'a', ' ');
        ...
        src1 = getLengthDummyBlock(NGramSynonymTokenizer.BUFFER_SIZE - 2, 'a',
                ' ');
        String src2 = getLengthDummyBlock(
                NGramSynonymTokenizer.BUFFER_SIZE - 2, 'a', ' ');
        tokenizer = getTokenizer(src1 + src2);
        assertBlocks(tokenizer, new int[] { 0,
                NGramSynonymTokenizer.BUFFER_SIZE - 1 },
                src1.substring(0, NGramSynonymTokenizer.BUFFER_SIZE - 2),
        ...
        tokenizer = getTokenizer(src1 + src2);
        assertBlocks(tokenizer, new int[] { 0,
                NGramSynonymTokenizer.BUFFER_SIZE },
                src1.substring(0, NGramSynonymTokenizer.BUFFER_SIZE - 1),
                src2.substring(0, NGramSynonymTokenizer.BUFFER_SIZE - 2));
        ...
        tokenizer = getTokenizer(src1 + src2);
        assertBlocks(tokenizer, new int[] { 0,
                NGramSynonymTokenizer.BUFFER_SIZE + 2 },
                src1.substring(0, NGramSynonymTokenizer.BUFFER_SIZE + 1),
                src2.substring(0, NGramSynonymTokenizer.BUFFER_SIZE - 2));
        ...
        tokenizer = getTokenizer(src1 + src2);
        assertBlocks(tokenizer, new int[] { 0,
                NGramSynonymTokenizer.BUFFER_SIZE + 4 },
                src1.substring(0, NGramSynonymTokenizer.BUFFER_SIZE + 2),
                src2.substring(0, NGramSynonymTokenizer.BUFFER_SIZE - 2));
    }

    private NGramSynonymTokenizer getTokenizer(final String input)
            throws IOException {
        final NGramSynonymTokenizer tokenizer = new NGramSynonymTokenizer(
                new StringReader(input), NGramSynonymTokenizer.DEFAULT_N_SIZE,
                NGramSynonymTokenizer.DEFAULT_DELIMITERS, false, true, null);
        tokenizer.reset();
        return tokenizer;
    }

    private void assertBlocks(final NGramSynonymTokenizer tokenizer,
            final String expBlkStarts, final String... expBlocks)
            throws Exception {
    ...
    private void assertBlocks(final NGramSynonymTokenizer tokenizer,
            final int[] expBlkStarts, final String... expBlocks)
            throws Exception {
    ...
    private String getLengthDummyBlock(final int length, final char blockChar,
            final char... eobChars) {
    ...
    public void testMyTokensComparator() throws Exception {
        final PriorityQueue<MyToken> pq = new PriorityQueue<MyToken>(10,
                new NGramSynonymTokenizer.MyTokensComparator());
    ...
    public void testNullSynonyms() throws Exception {
        ...
        TokenStream stream = a.tokenStream("f", new StringReader("ショッピング"));
        stream.reset();
        assertTokenStream(stream,
                "シ,0,1,1/ョ,1,2,1/ッ,2,3,1/ピ,3,4,1/ン,4,5,1/グ,5,6,1");

        a = new NGramSynonymTokenizerTestAnalyzer(2);
        stream = a.tokenStream("f", new StringReader("ショッピング"));
        stream.reset();
        assertTokenStream(stream,
                "ショ,0,2,1/ョッ,1,3,1/ッピ,2,4,1/ピン,3,5,1/ング,4,6,1");
        stream.close();
        stream = a.tokenStream("f", new StringReader("シ"));
        stream.reset();
        assertTokenStream(stream, "シ,0,1,1");
        stream.close();
        stream = a.tokenStream("f", new StringReader("ショ"));
        stream.reset();
        assertTokenStream(stream, "ショ,0,2,1");

        a = new NGramSynonymTokenizerTestAnalyzer(3);
        stream = a.tokenStream("f", new StringReader("ショッピング"));
        stream.reset();
        assertTokenStream(stream, "ショッ,0,3,1/ョッピ,1,4,1/ッピン,2,5,1/ピング,3,6,1");

        a = new NGramSynonymTokenizerTestAnalyzer(4);
        stream = a.tokenStream("f", new StringReader("ショッピング"));
        stream.reset();
        assertTokenStream(stream, "ショッピ,0,4,1/ョッピン,1,5,1/ッピング,2,6,1");

        a = new NGramSynonymTokenizerTestAnalyzer(5);
        stream = a.tokenStream("f", new StringReader("ショッピング"));
        stream.reset();
        assertTokenStream(stream, "ショッピン,0,5,1/ョッピング,1,6,1");

        a = new NGramSynonymTokenizerTestAnalyzer(6);
        stream = a.tokenStream("f", new StringReader("ショッピング"));
        stream.reset();
        assertTokenStream(stream, "ショッピング,0,6,1");
    ...
    public void testSingleSynonymIgnoreCase() throws Exception {
        final Analyzer a = new NGramSynonymTokenizerTestAnalyzer(2, false,
                "A,AA,AAA");
    ...
    public void testMultipleSynonyms() throws Exception {
        Analyzer a = new NGramSynonymTokenizerTestAnalyzer(1, false,
                "a,aa/b,bb");
    ...
    public void testMultipleSynonymsExpand() throws Exception {
        ...
        stream.reset();
        assertTokenStream(stream,
                "a,0,1,1/aa,0,1,0/b,1,2,1/bb,1,2,0/a,2,3,1/aa,2,3,0/bb,3,5,1/b,3,5,0");

        a = new NGramSynonymTokenizerTestAnalyzer(1, true, "a,aa/b,bb/c,cc");
        stream = a.tokenStream("f", new StringReader("cba"));
        stream.reset();
        assertTokenStream(stream,
                "c,0,1,1/cc,0,1,0/b,1,2,1/bb,1,2,0/a,2,3,1/aa,2,3,0");
    }
    ...
    public void testPrevStrSingleSynonymExpand1() throws Exception {
        ...
        stream.reset();
        assertTokenStream(stream,
                "e,0,1,1/d,1,2,1/c,2,3,1/b,3,4,1/a,4,5,1/aa,4,5,0");
    }
    ...
    public void testPrevStrSingleSynonymExpand2() throws Exception {
        ...
        stream.reset();
        assertTokenStream(stream,
                "ed,0,2,1/dc,1,3,1/cb,2,4,1/b,3,4,0/a,4,5,1/aa,4,5,0");
    }
    ...
    public void testPrevStrSingleSynonymExpand3() throws Exception {
        ...
        stream.reset();
        assertTokenStream(stream,
                "edc,0,3,1/dcb,1,4,1/cb,2,4,0/b,3,4,0/a,4,5,1/aa,4,5,0");
    }
    ...
    public void testPrevStrSingleSynonymExpand4() throws Exception {
        ...
        stream.reset();
        assertTokenStream(stream,
                "edcb,0,4,1/dcb,1,4,0/cb,2,4,0/b,3,4,0/a,4,5,1/aa,4,5,0");

        a = new NGramSynonymTokenizerTestAnalyzer(4, true, "a,aa");
        stream = a.tokenStream("f", new StringReader("fedcba"));
        stream.reset();
        assertTokenStream(stream,
                "fedc,0,4,1/edcb,1,5,1/dcb,2,5,0/cb,3,5,0/b,4,5,0/a,5,6,1/aa,5,6,0");
    }
    ...
    public void testAfterStrSingleSynonymExpand1() throws Exception {
        ...
        stream.reset();
        assertTokenStream(stream,
                "a,0,1,1/aa,0,1,0/b,1,2,1/c,2,3,1/d,3,4,1/e,4,5,1");
    }
    ...
    public void testAfterStrSingleSynonymExpand2() throws Exception {
        ...
        stream.reset();
        assertTokenStream(stream,
                "a,0,1,1/aa,0,1,0/b,1,2,1/bc,1,3,0/cd,2,4,1/de,3,5,1");
    }
    ...
    public void testAfterStrSingleSynonymExpand3() throws Exception {
        ...
        stream.reset();
        assertTokenStream(stream,
                "a,0,1,1/aa,0,1,0/b,1,2,1/bc,1,3,0/bcd,1,4,0/cde,2,5,1");
    }
    ...
    public void testAfterStrSingleSynonymExpand4() throws Exception {
        ...
        stream.reset();
        assertTokenStream(stream,
                "a,0,1,1/aa,0,1,0/b,1,2,1/bc,1,3,0/bcd,1,4,0/bcde,1,5,0");

        a = new NGramSynonymTokenizerTestAnalyzer(4, true, "a,aa");
        stream = a.tokenStream("f", new StringReader("abcdef"));
        stream.reset();
        assertTokenStream(stream,
                "a,0,1,1/aa,0,1,0/b,1,2,1/bc,1,3,0/bcd,1,4,0/bcde,1,5,0/cdef,2,6,1");
    }
    ...
    public void testSandwichStr1() throws Exception {
        ...
        stream.reset();
        assertTokenStream(stream,
                "a,0,1,1/b,1,2,1/c,2,3,1/d,3,4,1/e,4,5,1/a,5,6,1");
    }
    ...
    public void testSandwichStr3() throws Exception {
        ...
        stream.reset();
        assertTokenStream(stream,
                "a,0,1,1/bcd,1,4,1/cde,2,5,1/def,3,6,1/a,6,7,1");
    }
    ...
    public void testSandwichStrExpand1() throws Exception {
        ...
        stream.reset();
        assertTokenStream(stream,
                "a,0,1,1/aa,0,1,0/b,1,2,1/b,2,3,1/a,3,4,1/aa,3,4,0");

        a = new NGramSynonymTokenizerTestAnalyzer(1, true, "a,aa");
        stream = a.tokenStream("f", new StringReader("abcda"));
        stream.reset();
        assertTokenStream(stream,
                "a,0,1,1/aa,0,1,0/b,1,2,1/c,2,3,1/d,3,4,1/a,4,5,1/aa,4,5,0");

        a = new NGramSynonymTokenizerTestAnalyzer(1, true, "a,aa");
        stream = a.tokenStream("f", new StringReader("abcdea"));
        stream.reset();
        assertTokenStream(stream,
                "a,0,1,1/aa,0,1,0/b,1,2,1/c,2,3,1/d,3,4,1/e,4,5,1/a,5,6,1/aa,5,6,0");
    }
    ...
    public void testSandwichStrExpand2() throws Exception {
        ...
        stream.reset();
        assertTokenStream(stream,
                "a,0,1,1/aa,0,1,0/b,1,2,1/bb,1,3,0/b,2,3,0/a,3,4,1/aa,3,4,0");

        a = new NGramSynonymTokenizerTestAnalyzer(2, true, "a,aa");
        stream = a.tokenStream("f", new StringReader("abcda"));
        stream.reset();
        assertTokenStream(stream,
                "a,0,1,1/aa,0,1,0/b,1,2,1/bc,1,3,0/cd,2,4,1/d,3,4,0/a,4,5,1/aa,4,5,0");

        a = new NGramSynonymTokenizerTestAnalyzer(2, true, "a,aa");
        stream = a.tokenStream("f", new StringReader("abcdea"));
        stream.reset();
        assertTokenStream(stream,
                "a,0,1,1/aa,0,1,0/b,1,2,1/bc,1,3,0/cd,2,4,1/de,3,5,1/e,4,5,0/a,5,6,1/aa,5,6,0");
    }
    ...
    public void testSandwichStrExpand3() throws Exception {
        ...
        stream.reset();
        assertTokenStream(stream,
                "a,0,1,1/aa,0,1,0/b,1,2,1/bc,1,3,0/bcd,1,4,0/cd,2,4,0/d,3,4,0/a,4,5,1/aa,4,5,0");
    ...
    public void testSandwichSynonym1() throws Exception {
        ...
        stream.reset();
        assertTokenStream(stream,
                "d,0,1,1/c,1,2,1/b,2,3,1/a,3,4,1/b,4,5,1/c,5,6,1/d,6,7,1");

        a = new NGramSynonymTokenizerTestAnalyzer(1, false, "a,aa");
        stream = a.tokenStream("f", new StringReader("edcbabcde"));
        stream.reset();
        assertTokenStream(stream,
                "e,0,1,1/d,1,2,1/c,2,3,1/b,3,4,1/a,4,5,1/b,5,6,1/c,6,7,1/d,7,8,1/e,8,9,1");
    }
    ...
    public void testSandwichSynonym2() throws Exception {
        ...
        stream.reset();
        assertTokenStream(stream,
                "ed,0,2,1/dc,1,3,1/cb,2,4,1/a,4,5,1/bc,5,7,1/cd,6,8,1/de,7,9,1");
    }
    ...
    public void testSandwichSynonym3() throws Exception {
        ...
        stream.reset();
        assertTokenStream(stream,
                "edc,0,3,1/dcb,1,4,1/a,4,5,1/bcd,5,8,1/cde,6,9,1");

        a = new NGramSynonymTokenizerTestAnalyzer(3, false, "a,aa");
        stream = a.tokenStream("f", new StringReader("fedcbabcdef"));
        stream.reset();
        assertTokenStream(stream,
                "fed,0,3,1/edc,1,4,1/dcb,2,5,1/a,5,6,1/bcd,6,9,1/cde,7,10,1/def,8,11,1");
    }
    ...
    public void testSandwichSynonym4() throws Exception {
        ...
        stream.reset();
        assertTokenStream(stream,
                "fedc,0,4,1/edcb,1,5,1/a,5,6,1/bcde,6,10,1/cdef,7,11,1");
    }
    ...
    public void testSandwichSynonymExpand1() throws Exception {
        ...
        stream.reset();
        assertTokenStream(stream,
                "b,0,1,1/b,1,2,1/a,2,3,1/aa,2,3,0/b,3,4,1/b,4,5,1");

        a = new NGramSynonymTokenizerTestAnalyzer(1, true, "a,aa");
        stream = a.tokenStream("f", new StringReader("dcbabcd"));
        stream.reset();
        assertTokenStream(stream,
                "d,0,1,1/c,1,2,1/b,2,3,1/a,3,4,1/aa,3,4,0/b,4,5,1/c,5,6,1/d,6,7,1");
    ...
    public void testSandwichSynonymExpand2() throws Exception {
        ...
        stream.reset();
        assertTokenStream(stream,
                "bb,0,2,1/b,1,2,0/a,2,3,1/aa,2,3,0/b,3,4,1/bb,3,5,0");

        a = new NGramSynonymTokenizerTestAnalyzer(2, true, "a,aa");
        stream = a.tokenStream("f", new StringReader("dcbabcd"));
        stream.reset();
        assertTokenStream(stream,
                "dc,0,2,1/cb,1,3,1/b,2,3,0/a,3,4,1/aa,3,4,0/b,4,5,1/bc,4,6,0/cd,5,7,1");
    ...
    public void testSandwichSynonymExpand3() throws Exception {
        ...
        stream.reset();
        assertTokenStream(stream,
                "dcb,0,3,1/cb,1,3,0/b,2,3,0/a,3,4,1/aa,3,4,0/b,4,5,1/bc,4,6,0/bcd,4,7,0");
    ...
    public void testComplex1() throws Exception {
        ...
        stream.reset();
        assertTokenStream(stream,
                "c,0,1,1/c,1,2,1/a,2,3,1/b,3,4,1/c,4,5,1/c,5,6,1/a,6,7,1");

        a = new NGramSynonymTokenizerTestAnalyzer(1, false, "a,aa/b,bb");
        stream = a.tokenStream("f", new StringReader("edcabcdea"));
        stream.reset();
        assertTokenStream(stream,
                "e,0,1,1/d,1,2,1/c,2,3,1/a,3,4,1/b,4,5,1/c,5,6,1/d,6,7,1/e,7,8,1/a,8,9,1");
    ...
    public void testComplex2() throws Exception {
        Analyzer a = new NGramSynonymTokenizerTestAnalyzer(2, false,
                "a,aa/b,bb");
        TokenStream stream = a.tokenStream("f", new StringReader("cabca"));
        stream.reset();
        assertTokenStream(stream, "c,0,1,1/a,1,2,1/b,2,3,1/c,3,4,1/a,4,5,1");
        ...
        stream.reset();
        assertTokenStream(stream,
                "ed,0,2,1/dc,1,3,1/a,3,4,1/b,4,5,1/cd,5,7,1/de,6,8,1/a");
    ...
    public void testComplex3() throws Exception {
        Analyzer a = new NGramSynonymTokenizerTestAnalyzer(3, false,
                "a,aa/b,bb");
        TokenStream stream = a.tokenStream("f", new StringReader("cabca"));
        stream.reset();
        assertTokenStream(stream, "c,0,1,1/a,1,2,1/b,2,3,1/c,3,4,1/a,4,5,1");
        ...
        stream.reset();
        assertTokenStream(stream,
                "fed,0,3,1/edc,1,4,1/a,4,5,1/b,5,6,1/cde,6,9,1/def,7,10,1/a,10,11,1");
    ...
    public void testComplex4() throws Exception {
        Analyzer a = new NGramSynonymTokenizerTestAnalyzer(4, false,
                "a,aa/b,bb");
        TokenStream stream = a.tokenStream("f", new StringReader("cabca"));
        stream.reset();
        assertTokenStream(stream, "c,0,1,1/a,1,2,1/b,2,3,1/c,3,4,1/a,4,5,1");
        ...
        stream.reset();
        assertTokenStream(stream,
                "fedc,0,4,1/a,4,5,1/b,5,6,1/cdef,6,10,1/a,10,11,1");

        a = new NGramSynonymTokenizerTestAnalyzer(4, false, "a,aa/b,bb");
        stream = a.tokenStream("f", new StringReader("gfedcabcdefga"));
        stream.reset();
        assertTokenStream(stream,
                "gfed,0,4,1/fedc,1,5,1/a,5,6,1/b,6,7,1/cdef,7,11,1/defg,8,12,1/a,12,13,1");
    }
    ...
    public void testComplexExpand1() throws Exception {
        ...
        stream.reset();
        assertTokenStream(stream,
                "c,0,1,1/a,1,2,1/aa,1,2,0/b,2,3,1/bb,2,3,0/c,3,4,1/a,4,5,1/aa,4,5,0");
    ...
    private void assertTokenStream(final TokenStream stream,
            final String expectedStream) throws Exception {
    ...
    private void assertAttribute(final int count, final String type,
            final String expected, final String actual) throws Exception {
        if (expected.equals("[null]")) {
            assertNull(
                    String.format(
                            "%s is invalid at token %d, expected : \"%s\" != actual : \"%s\"",
    ...
    private void assertAttribute(final int count, final String type,
            final int expected, final int actual) throws Exception {
        assertEquals(
                String.format(
                        "%s is invalid at token %d, expected : \"%d\" != actual : \"%d\"",
                        type, count, expected, actual), expected, actual);
    }

    public static final class NGramSynonymTokenizerTestAnalyzer extends
            Analyzer {
        ...
        public NGramSynonymTokenizerTestAnalyzer(final int n,
                final boolean expand, final String synonyms) {
        ...
        protected TokenStreamComponents createComponents(
            ...
            final Tokenizer source = new NGramSynonymTokenizer(reader, n,
                    delimiters, expand, true, synonyms);
            return new TokenStreamComponents(source);
        }

        private SynonymMap getSynonymMap(final String synonyms) {
            if (synonyms != null) {
                final SolrSynonymParser parser = new SolrSynonymParser(true,
                        true, NGramSynonymTokenizerFactory.getAnalyzer(true));
                try {
